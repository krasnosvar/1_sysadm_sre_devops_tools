...
---------------------------------------------------------------------------------------------
#gitlab-api
curl --insecure https://gitlab.corp.domain.ru/api/v4/projects | python -m json.tool> gitlab_api.json

#USERS in commnad line
#Unlock user
...
User.find_by_email("user@domain.local") do |i|
    i.admin = true
    i.save
end
#or
...
no_proxy="localhost,127.0.0.0/24,gitlab.corp.domain.ru"
2. export certifitate fron gitlab site with firefox
3. copy-rename certifitate to gitlab-runner directory 
mv /home/den/Downloads/v00gitlab_corp_domain_ru.crt /etc/gitlab-runner/certs/gitlab_corp_domain_ru.crt
4. register gitlab-runner with command
sudo gitlab-runner register --tls-ca-file /etc/gitlab-runner/certs/gitlab_corp_domain_ru.crt
...
[[runners]]
  ...
  url = "https://gitlab.corp.domain.ru/"
  token = "token-here"
  executor = "docker"
  tls-ca-file = "/etc/gitlab-runner/certs/gitlab.corp.domain.ru.crt"
  [runners.docker]
    tls_verify = false
...
#add registry to gitlab
mkdir -p /etc/docker/certs.d/v00rpm-dr.corp.domain.ru/
cp /home/user/v00rpm-dr_corp_domain_ru.crt /etc/docker/certs.d/v00rpm-dr.corp.domain.ru/
...
# workaround mask variables with sensitive characters
# https://devops.stackexchange.com/questions/8627/how-to-pass-gitlab-env-var-with-sensitive-characters-to-bash-script
$ echo 'aaa$bbb*%' | base64
YWFhJGJiYiolCg==
# Then store this value in project Variables as "sensitive_var".
