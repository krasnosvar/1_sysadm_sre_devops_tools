##### Most used ( by myself ) linux build-in commands and utilities 
...
1. ```1_kernel_hard_system_ps``` - kernel and hardware utils (cpu, sysctl, procfs, devfs/udev, systemd/sysV, ntp/chrony, lsof, wi-fi)
2. ```2_disks_mount_lvm_nfs``` - disk utils: dd, parted, lvm, mount, nfs, multipath, smartctl, NVMe/SSD wear, swap
...
6. ```6_text_manipulation_utils``` - text tools: awk, sed, grep, additional safe grep/rg and jq/yq v4 examples, cut, sort, xargs, delta
